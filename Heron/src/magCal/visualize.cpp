...
FLASHMEM void apply_calibration(int16_t rawx, int16_t rawy, int16_t rawz, Point_t* out)
{
  float x, y, z;
...
FLASHMEM void quad_to_rotation(const Quaternion_t* quat, float* rmatrix)
{
  float qw = quat->q0;
  float qx = quat->q1;
  float qy = quat->q2;
...
FLASHMEM void rotate(const Point_t* in, Point_t* out, const float* rmatrix)
{
