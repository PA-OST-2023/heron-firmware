...
extern "C"
{
  ...
  void apply_calibration(int16_t rawx, int16_t rawy, int16_t rawz,
                         Point_t* out);
  void quad_to_rotation(const Quaternion_t* quat, float* rmatrix);
  void rotate(const Point_t* in, Point_t* out, const float* rmatrix);
  ...
  typedef struct
  {
    ...
    float matA[10]
              [10];    // scratch 10x10 matrix used by calibration algorithms
    float matB[10]
              [10];    // scratch 10x10 matrix used by calibration algorithms
    ...
    int8_t
      ValidMagCal;    // integer value 0, 4, 7, 10 denoting both valid calibration and solver used
  ...
  void eigencompute(float A[][10], float eigval[], float eigvec[][10],
                    int8_t n);
...
#define USE_MAHONY_FUSION
  ...
  void fusion_update(const AccelSensor_t* Accel, const MagSensor_t* Mag,
