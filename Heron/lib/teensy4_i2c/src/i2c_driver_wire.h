...
class I2CDriverWire : public Stream {
 public:
  ...
  static constexpr const uint32_t timeout_millis_default = 100;
  ...
  uint8_t requestFrom(int address, int quantity, int stop = true, bool verbose = true);
  ...
  inline void onReceive(void (*function)(int len)) { on_receive = function; }
  ...
  inline void onRequest(void (*function)()) { on_request = function; }
  ...
  inline int getLastAddress() { return last_address_called; }

  // Override various functions to avoid ambiguous calls
  inline void begin(int address) { begin((uint8_t)address); }
  inline void begin(int first_address, int second_address) {
    begin((uint8_t)first_address, (uint8_t)second_address);
  }
  inline void beginRange(int first_address, int last_address) {
    beginRange((uint8_t)first_address, (uint8_t)last_address);
  }
