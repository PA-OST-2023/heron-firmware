...
result format()
{
  ...
  memcpy(thisInfo.idStr, OSFS_ID_STR, 4);
...
{
  if(address < startOfEEPROM || address > endOfEEPROM)
    return result::UNCAUGHT_OOR;
  if(address + num < startOfEEPROM || address + num > endOfEEPROM)
    return result::UNCAUGHT_OOR;
